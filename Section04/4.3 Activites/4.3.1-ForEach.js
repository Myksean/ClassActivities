...
const data = [
  ...
  data.forEach(cities => console.log([cities.address.city]));
  ...
  const arrOfString = [];
  data.forEach(item =>  {
      let catchString = `${item.company.catchPhrase} ${item.company.bs}`;
  ...
  classes.forEach((htmlElement, i) => 
    htmlElement.textContent = `text content is changed, class week number ${i}`
  );